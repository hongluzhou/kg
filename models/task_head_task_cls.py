...
class Task_Head(nn.Module):
    def __init__(self, args, logger):
        ...
        if args.time_position_embedding_type == 'absolute_learned_1D':
            ...
            self.time_embed_layer = PositionEmbeddingAbsoluteLearned_1D(
                args.max_time_ids_embed, args.model_task_cls_tx_hidden_dim)
        
        elif args.time_position_embedding_type == 'fixed_sinusoidal_1D':
            ...
            raise ValueError(f"not supported {self.args.time_position_embedding_type}")
        ...
        self.classifier = build_mlp(
            ...
            hidden_dims=[args.model_task_cls_tx_hidden_dim//4], 
    ...
    def forward(self, video_feats, video_mask):
        ...
        if self.args.time_position_embedding_type == 'absolute_learned_1D':
            time_ids = torch.arange(1, T+1, device=device).repeat(B, 1)
            time_seq = self.time_embed_layer(time_ids) 
        elif self.args.time_position_embedding_type == 'fixed_sinusoidal_1D':
